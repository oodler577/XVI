...
%zeropage basicsafe
%option no_sysinit
; C64 uses the target default PETSCII string/char encoding.
; TARGET: external C-preprocessor selection.
; The Makefile should pass exactly one target define, for example -DTARGET_X16
; or -DTARGET_C64. Undefined targets default to 0 so #if blocks stay exclusive.
%import textio
%import strings
%import conv
%import syslib
...
view {
  ; C64: conservative 40x25 editor geometry.
  const ubyte LEFT_MARGIN = 4
  const ubyte RIGHT_MARGIN = 39
  const ubyte HEIGHT = 22
  const ubyte TOP_LINE = 1
  const ubyte MIDDLE_LINE = 11
  const ubyte BOTTOM_LINE = 22
  const ubyte FOOTER_LINE = 24
  ; Full line width, and editable line width without the gutter.
  ; Names remain legacy/X16-shaped so the editor core does not change yet.
  str BLANK_LINE79 = " " * 39
  str BLANK_LINE76 = " " * 36
  str EMPTY_LINE_MARK = ".   "
...
cursor {
  ...
  sub restore_current_char() {
    ...
    txt.plot(c,r)
    if saved_char == $22 {
      ; C64: no X16 ISO quote-mode escape needed here.
    }
...
command {
  ...
  CMDINPUT:
    ...
    cmdchar = main.normalize_key(cmdchar)
...
main {
  ...
  sub splash() {
    ; C64 splash: compact 40-column intro, PETSCII-safe.
    ; Use lowercase character set + simple PETSCII text so the splash is
    ; readable on stock C64/VICE without relying on ISO/X16 screen behavior.
    txt.lowercase()
    txt.clear_screen()
    ...
    say("")
    say(":e file   load file")
    say(":q        quit")
    say("i         insert mode")
    say("r         replace mode")
    say("")
    say("press i or r to start editing")
  }
  sub clear_splash() {
    txt.clear_screen()
    txt.plot(0,view.TOP_LINE)
  ...
  ubyte row
  ubyte numbers
  sub normalize_key(ubyte k) -> ubyte {
    ; C64 physical left-arrow key near 1: use as editor ESC/cancel.
    if k == $5f {
      return $1b
    }
    ; C64 RUN/STOP: also use as editor ESC/cancel, useful in VICE/dev.
    if k == $03 {
      return $1b
    }
    ; C64/VICE often returns unshifted letters as uppercase PETSCII.
    ; Normalize the common editor-command/text path to ASCII-ish lowercase.
    if k >= $41 and k <= $5a {
      return k + $20
    }
    return k
  ...
  sub key_mode_init() {
    ; On splash/INIT: treat ESC as a prefix so ESC+R / ESC+i is instant.
      ...
      when char {
        'R','r' -> { goto main.key_mode_nav.RLOOP2 }
  ...
  sub key_mode_nav() {
    when char {
      ...
      'k' -> cursor_up_on_k() ; vi UP
      'j' -> cursor_down_on_j() ; vi DOWN
      'h' -> cursor_left_on_h() ; vi LEFT
      'l' -> cursor_right_on_l() ; vi RIGHT
      ; C64 raw CBM cursor and control-key aliases.
      $91 -> cursor_up_on_k() ; UP
      $11 -> cursor_down_on_j() ; DOWN
      $9d, $14, $08 -> cursor_left_on_h() ; LEFT/backspace variants
      $20, $1d -> cursor_right_on_l() ; SPACE/right arrow
  ...
  sub redraw_line(^^Line addr, ubyte r) -> ubyte {
    ...
    strings.rstrip(tmp) ; <- to get rid of stray CR or LF, but is this necessary?
    prints(tmp)
  ...
  sub update_tracker () {
    ...
    if flags.VERBOSE_STATUS == false {
      ...
      txt.plot(view.RIGHT_MARGIN-12, view.FOOTER_LINE)
      printW(Y - view.TOP_LINE + 1)
      prints(",")
      printW(X - view.LEFT_MARGIN + 1)
      txt.plot(view.RIGHT_MARGIN-3, view.FOOTER_LINE)
      ...
      txt.plot(view.RIGHT_MARGIN-9, view.FOOTER_LINE)
      if flags.UNSAVED == true {
        txt.color2($6,$1)
        prints("(UNSAVED)")
        txt.color2($1,$6)
  ...
  sub alert_noblock(str message, ubyte color1, ubyte color2) {
    ubyte length = strings.length(message)
    txt.plot(view.RIGHT_MARGIN-1-length, 0)
    txt.color2(color1, color2)
    prints(message)
    txt.color2($1, $6) ; sets text back to default, white on blue
  }
...
txt {
  %option merge
  ; C64 compatibility shims for X16 textio conveniences used above.
  ; C64 textio has color(), but not color2()/iso()/iso_off().
  ...
  ; Conservative compile-only placeholders. C64 screen-RAM scroll comes next.
